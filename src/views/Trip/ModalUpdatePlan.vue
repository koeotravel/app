<template>
  ...
  >
    <div
      class="flex-auto measure-wide center card-brand-ns bg-white mv5-ns modal"
      @click.stop
    >
      <header class="pa3 bb b--shadow-brand flex items-center">
        <h3 class="ml4 mr2 mv0 f4 normal tc brand dib flex-auto">
          Update {{ currentPlan.type | capitalize }} Plan
        </h3>
        <XIcon
      ...
      <form @submit.prevent="handleUpdatePlan">
        <section class="flex flex-column pa3-ns pv3 ph2 ">
          <div class="w-100 flex flex-row items-center">
            <div class="w-50 mr3">
              <select
                ...
                class="pa3 br3 mb3 mb0-m mv3-l mr3 input-reset ba w-100"
              ...
              <div
                v-if="hasVenue"
                class="br2 f6 bw1 b--brand bg-white brand pa3 mt3-m mb3 mb0-m w-100"
              >
                <span class="flex items-center">
                  <span class="f5 truncate w-100 tl">
                    {{ currentPlan.venue.name }}
                  </span>
                </span>
              </div>
              <select
                v-if="!isEvent"
                v-model="currentPlan.end_date"
                class="pa3 br3 mb3 mt3-m mb0-m input-reset ba w-100"
              >
                <option
                  v-for="day in days"
                  v-show="day.date >= currentPlan.date"
                  :key="day.id"
                  :value="day.date"
                >
                  {{ day.date | humanPlanDate }}
                </option>
              </select>
            </div>
            <div class="card-brand overflow-hidden w-50 h4">
              <div
                :style="{'background':'url('+currentPlan.img+') center'}"
                class="h4 cover flex flex-column"
              >
                <div
                  v-if="currentPlan.url"
                  ...
                  <span class="bg-black-50 pv1 ph2 white br2 h-100 flex items-center">
                    ...
                    <span class="dib">
                      <a
                        ...
                        target="_blank"
                        class="f7 ma0 normal db link dim brand pointer"
                      >
                        View Website
                      </a>
                    </span>
                ...
                <div class="flex flex-auto pa2 justify-start items-end">
                  <button
                    v-if="isEvent"
                    type="button"
                    class="br2 f6 ba b--solid bw1 pointer pa2 brand b--brand o-80 mr2"
                    @click="handleToggleVenueImg('left')"
                  >
                    <ChevronLeftIcon />
                  </button>
                  <button
                    v-if="isEvent"
                    type="button"
                    class="br2 f6 ba b--solid bw1 pointer pa2 brand b--brand o-80"
                    @click="handleToggleVenueImg('right')"
                  >
                    <ChevronRightIcon />
                  </button>
                </div>
          ...
          <div class="w-100 pt3 flex flex-column items-center">
            <VInput
              v-show="currentPlan.type === 'transportation'"
              v-model="currentPlan.name"
              placeholder="🛃  Add the transportation name here..."
            >
              Transportation
            </VInput>
            <VInput
              v-show="currentPlan.type === 'lodging'"
              v-model="currentPlan.name"
              placeholder="🏨  Add the lodging name here..."
            >
              Lodging Name
            </VInput>
            <VInput
              v-show="currentPlan.type === 'event'"
              v-model="currentPlan.name"
              placeholder="🎭  Add the event name here..."
            >
              Event Name
            </VInput>
            ...
            <VInput
              ...
              :disabled="currentPlan.venue && Object.keys(currentPlan.venue).length !== 0"
            >
              Address
            </VInput>
            ...
            <div class="flex flex-auto w-100 pb3 flex-row-ns flex-column">
              ...
              <span class="br3 ba flex-auto flex flex-row b--light-gray br--right-ns items-center">
                ...
                <VButton
                  type="button"
                  btn-type="tertiary"
                  custom-class="h2 w2 mh2"
                  style="padding:4px"
                  :on-click="expandNotes"
                >
                  <Maximize2Icon
                    v-if="!textArea"
                    class="w1 h1"
