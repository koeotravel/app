<template>
  <form class="ph4-l ph3-m tl" @submit.prevent>
    <div class="card-brand trip-description mb4 overflow-hidden">
      <div class="h5 cover flex flex-column" :style="{'background':'url('+currentTrip.img+') center'}">
        <div class="flex justify-end">
          <span class="bg-black-50 pv1 ph2 white br2 h-100">
            <h6 class="f7 ma0 normal">Image Powered by
              ...
              <a :href="currentTrip.img_author_url" target="_blank" class="link dim brand pointer"> {{ currentTrip.img_author }} </a>
            </h6>
          </span>
        </div>
        <span class="flex-auto"></span>
        <v-avatar-stack :users="party"></v-avatar-stack>
      </div>
    </div>
  </form>
</template>
